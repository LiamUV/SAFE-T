...
try
    ...
    txPluto1 = sdrtx('Pluto', 'RadioID', 'usb:0', ...  % Using first available USB device
                     'CenterFrequency', 406.025e6, ...
                     'BasebandSampleRate', 1e6, ...
                     'Gain', -10);
    disp('Transmitting 406.025 MHz signal using Pluto 1...');
catch ME
    ...
    try
        ...
        txPluto1 = sdrtx('Pluto', 'RadioID', 'usb:1', ...  % Using second available USB device
                         'CenterFrequency', 406.025e6, ...
                         'BasebandSampleRate', 1e6, ...
                         'Gain', -10);
        disp('Transmitting 406.025 MHz signal using Pluto 2...');
    catch ME2
...
while true
    try
        txPluto1(signal);  % Transmit signal
    catch ME2
